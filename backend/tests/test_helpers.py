...
from app import split_into_weeks
from app import split_by_month_or_period
from app import is_vacation_day
from app import is_weekend
...
def test_get_week_schedule():
    ...
    end_date = "2023-10-07"
    expected_schedule = [
        "Dim. 01-10", "Lun. 02-10", "Mar. 03-10", "Mer. 04-10", 
        "Jeu. 05-10", "Ven. 06-10", "Sam. 07-10"
    ]
...
def test_split_into_weeks_single_week():
    ...
    expected_output = [["Lun. 01-01", "Mar. 02-01", "Mer. 03-01", "Jeu. 04-01", "Ven. 05-01", "Sam. 06-01", "Dim. 07-01"]]
    assert split_into_weeks(week_schedule) == expected_output

def test_split_into_weeks_multiple_weeks():
    week_schedule = [
        "Lun. 01-01", "Mar. 02-01", "Mer. 03-01", "Jeu. 04-01", "Ven. 05-01", "Sam. 06-01", "Dim. 07-01",
        "Lun. 08-01", "Mar. 09-01", "Mer. 10-01", "Jeu. 11-01", "Ven. 12-01", "Sam. 13-01", "Dim. 14-01"
    ]
    expected_output = [
        ["Lun. 01-01", "Mar. 02-01", "Mer. 03-01", "Jeu. 04-01", "Ven. 05-01", "Sam. 06-01", "Dim. 07-01"],
        ["Lun. 08-01", "Mar. 09-01", "Mer. 10-01", "Jeu. 11-01", "Ven. 12-01", "Sam. 13-01", "Dim. 14-01"]
...
def test_split_by_month_or_period_multiple_months():
    week_schedule = [
        "Lun. 29-01", "Mar. 30-01", "Mer. 31-01", "Jeu. 01-02", "Ven. 02-02",
        "Sam. 03-02", "Dim. 04-02", "Lun. 05-02", "Mar. 06-02", "Mer. 07-02"
    ]
    expected = [
        ["Lun. 29-01", "Mar. 30-01", "Mer. 31-01"],
        ["Jeu. 01-02", "Ven. 02-02", "Sam. 03-02", "Dim. 04-02", "Lun. 05-02", "Mar. 06-02", "Mer. 07-02"]
    ]
    result = split_by_month_or_period(week_schedule)
    assert result == expected
...
def test_split_by_month_or_period_edge_case():
    week_schedule = [
        "Lun. 31-12", "Mar. 01-01", "Mer. 02-01", "Jeu. 03-01", "Ven. 04-01",
        "Sam. 05-01", "Dim. 06-01", "Lun. 07-01", "Mar. 08-01", "Mer. 09-01"
    ]
    expected = [
        ["Lun. 31-12"],
        ["Mar. 01-01", "Mer. 02-01", "Jeu. 03-01", "Ven. 04-01", "Sam. 05-01", "Dim. 06-01", "Lun. 07-01", "Mar. 08-01", "Mer. 09-01"]
    ]
    result = split_by_month_or_period(week_schedule)
    assert result == expected
